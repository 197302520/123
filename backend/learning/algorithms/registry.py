...
ALGORITHM_REGISTRY: list[dict[str, Any]] = [
    ...
    spec("paths.floyd", "Floyd 全源最短路", "d_ij=min(d_ij,d_ik+d_kj)", "用动态规划计算所有节点对最短路，并同时输出任意两点的完整路径节点序列与全网平均最短路径长度。", ["可同时观察全部节点对及其路径序列"], ["时间复杂度 O(n^3)"], parameters={"path_pair_limit": parameter("integer", 10_000, "输出完整路径节点序列的点对数量上限。", minimum=1, maximum=50_000)}, max_nodes=500, max_edges=20_000),
...
# 教学模块归属：前端按此把算法归入七个课程模块（与 seed_learning_content.MODULES 对应）。
ALGORITHM_MODULE: dict[str, str] = {
    "graph.validate": "network-basics",
    "text.extract": "network-basics",
    "export.graph": "network-basics",
    "model.er": "network-basics",
    "model.ws": "network-basics",
    "model.ba": "network-basics",
    "topology.summary": "network-measures",
    "paths.floyd": "network-measures",
    "clustering.coefficient": "network-measures",
    "centrality.degree": "network-measures",
    "centrality.closeness": "network-measures",
    "centrality.betweenness": "network-measures",
    "centrality.eigenvector": "network-measures",
    "centrality.pagerank": "network-measures",
    "centrality.hits": "network-measures",
    "centralization.degree": "network-measures",
    "embedding.ae": "network-measures",
    "embedding.cnn": "network-measures",
    "embedding.gcn": "network-measures",
    "embedding.gat": "network-measures",
    "community.kernighan_lin": "communities",
    "community.agglomerative": "communities",
    "community.divisive": "communities",
    "community.girvan_newman": "communities",
    "community.fast_newman": "communities",
    "community.louvain": "communities",
    "community.leiden": "communities",
    "community.lpa": "communities",
    "community.cpm": "communities",
    "community.lfm": "communities",
    "community.slpa": "communities",
    "community.compare": "communities",
    "opinion.degroot": "diffusion",
    "opinion.friedkin_johnsen": "diffusion",
    "opinion.deffuant": "diffusion",
    "opinion.hk": "diffusion",
    "robustness.attack": "robustness",
    "link_prediction.common_neighbors": "link-prediction",
    "link_prediction.jaccard": "link-prediction",
    "link_prediction.adamic_adar": "link-prediction",
    "link_prediction.resource_allocation": "link-prediction",
    "community.dynamic": "dynamic-networks",
}
...
ALGORITHM_REGISTRY.append(spec(
    "community.compare", "多算法模块度对比", "a*=argmax_a Q_a(G)",
    "一次运行对比 FN/Louvain/Leiden/LPA 等非重叠算法的模块度并给出最优筛选结论；小网络（≤60 节点）额外纳入二分、层次与 CPM/LFM/SLPA 重叠方法（重叠划分不参与模块度排序）。",
    ["聚合输出对比表、排名与最优结论，无需手动多次运行"],
    ["重叠划分的模块度不可直接比较；大网络自动精简小图专用算法"],
    graph_types=["undirected"],
    parameters={
        "resolution": deepcopy(COMMUNITY_PARAMETERS["resolution"]),
        "communities": deepcopy(COMMUNITY_PARAMETERS["communities"]),
    },
    max_nodes=2_000, max_edges=20_000,
))
...
ALGORITHM_REGISTRY.extend([
    ...
    spec("embedding.ae", "AE 属性自编码嵌入聚类", "Z=tanh([A|X]W_e), X̂=ZW_d", "在 CPU 上联合邻接矩阵与可选节点 features 属性训练自编码器，再对嵌入做 k-means。", ["可联合结构与节点属性"], ["未显式利用卷积局部性"], parameters={"clusters": parameter("integer", 2, "聚类数。", minimum=1, maximum=50), "embedding_dim": parameter("integer", 2, "嵌入维数。", minimum=1, maximum=64), "epochs": parameter("integer", 100, "训练轮数。", minimum=1, maximum=2_000), "learning_rate": parameter("number", 0.05, "学习率。", minimum=1e-5, maximum=1)}, max_nodes=1_000),
    ...
    spec("text.extract", "中文实体关系建网", "w_ij=cos(context_i,context_j), count_ij/max(count), or mean(BGE_ij)", "确定性预处理后生成可校正的实体/关系候选；同指实体（如“阿里”与“阿里巴巴”）按字符二元组余弦或包含关系的语义相似度合并为统一节点；重复有向节点对聚合为一条边，保留发生次数与候选索引，并按合并证据余弦、节点对计数归一化或 BGE 均值定权。", ["证据、偏移、发生次数与置信度可校正", "同指实体自动消歧合并"], ["规则模式可漏掉隐含关系；模型适配器需本地能力"], parameters={"text": parameter("string", "", "待抽取中文文本。"), "method": parameter("string", "rule", "实体/关系方法。", choices=["rule", "paddlenlp"]), "embedding": parameter("string", "cosine", "聚合边权方法。", choices=["cosine", "normalized", "bge"]), "merge_threshold": parameter("number", 0.6, "同义实体合并相似度阈值；0 表示关闭合并。", minimum=0, maximum=1), "model_path": parameter("string", "", "可选本地模型路径。")}, max_nodes=10_000),
    spec("export.graph", "标准图导出", "serialize(G,format)", "以确定顺序导出 Excel 工作簿（节点编号清单 + 邻接矩阵 + 边列表，Base64 编码下载）、JSON、CSV、GraphML、GEXF、GML、Pajek、边表或邻接矩阵。", ["格式广泛且可复现", "原生 XLSX 可被 Excel/WPS 直接打开"], ["部分文本格式不保留复杂属性"], parameters={"format": parameter("string", "json", "导出格式。", choices=["json", "csv", "xlsx", "graphml", "gexf", "gml", "pajek", "edgelist", "adjacency"])}),
])
...
for item in ALGORITHM_REGISTRY:
    item["module"] = ALGORITHM_MODULE.get(item["key"], "network-basics")
