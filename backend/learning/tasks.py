...
def execute_run_job(run_id: str) -> str:
    ...
    with RunLeaseHeartbeat(str(run.id), run.task_id):
        try:
            ...
            return Run.objects.only("status").get(pk=run_id).status
    updated = Run.objects.filter(pk=run_id, status=Run.Status.RUNNING).update(
...
def cleanup_expired_runs() -> int:
    ...
    Run.objects.filter(
        ...
        status=Run.Status.FAILED,
        error={"code": "queue_delivery_failed", "message": "任务队列投递失败，请重新提交。", "path": ""},
